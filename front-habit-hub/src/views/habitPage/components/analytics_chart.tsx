...
const HabitAnalyticsChart: React.FC<HabitAnalyticsChartProps> = ({ data }) => {
  ...
  const getFilteredData = () => {
    ...
    let filtered = data.filter(d => new Date(d.date) <= dateNow);

    if (range === 'week' || range === 'month') {
      const days = possibleRange[range];
      const from = new Date(dateNow);
      from.setDate(from.getDate() - days);
      return filtered.filter(d => new Date(d.date) >= from);
    }
  ...
  return (
    <div className="w-full p-4">
      <h3 className="font-bold text-lg mb-3">📊 Habit Value Over Time</h3>
