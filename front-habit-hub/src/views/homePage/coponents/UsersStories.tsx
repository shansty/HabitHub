...
const UsersStories: React.FC = () => {
    ...
    return (
        <section className="bg-blue-100 py-12 px-6 text-center">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-10 ">Stories Of Our Users</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
                {people.map((person, index) => (
                    <div key={index} className="bg-white p-6 shadow-md text-left">
                        <h3 className="text-xl font-semibold mb-2">{person.name}, {person.age}</h3>
                        <p className="text-indigo-500 font-medium mb-4">{person.role}</p>
