...
export class HabitOccurrenceService {
    ...
    generateOccurrences(
        ...
        for (let i = 0; i < habit.goalDuration; i++) {
            ...
            if (
                this.shouldIncludeDate(currentDate, scheduleType, scheduleData)
            ) {
                occurrences.push(
    ...
    generateOccurrencesForNewAttempt(
        habit: Habit,
        userId: string,
        date: Date
    ): HabitOccurrence[] {
    ...
    private shouldIncludeDate(
        date: Date,
        schedule: Schedule,
        data: { daysOfWeek?: number[]; daysOfMonth?: number[] }
    ): boolean {
