...
export class TasksService {
  ...
  private async countHabitProgressAndCreateHabitOccurences(eventsToUpdate: HabitEvent[], eventsToCreate: Partial<HabitEvent>[]): Promise<void>{
    const affectedHabitIds = [...new Set([...eventsToUpdate.map(e => e.habitId), ...eventsToCreate.map(e => e.habitId)])];
  ...
  async createAdditionalHabitOccurrences(habit: Habit, updated_progress: number, progress_without_fine: number): Promise<void> {
    const lostProgress = progress_without_fine - updated_progress
    if (!lostProgress) return;

    const newOccurrencesNeeded = Math.ceil((lostProgress / 100) * habit.goalDuration);
    const lastDate = habit.habitOccurrence.reduce((latest, current) => {
      return new Date(current.date) > new Date(latest.date) ? current : latest;
    }, habit.habitOccurrence[0])?.date ?? habit.startDate;

    const startFrom = new Date(lastDate);
    startFrom.setDate(startFrom.getDate() + 1);
    const futureOccurrences = this.habitOccurrenceService.generateOccurrencesFromDate(
      habit,
